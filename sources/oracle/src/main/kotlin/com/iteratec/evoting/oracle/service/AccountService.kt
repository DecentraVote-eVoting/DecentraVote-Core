...
class AccountService(
    ...
    fun registration(
        ...
        if (databaseService.accountExists(account.uuid!!)) {
            val dbEthAddress = databaseService.getEthAddrByUuid(account.uuid!!)
        ...
        databaseService.addAccountToDB(account)
    ...
    fun registerInitialDirector() {
        ...
        if (SolidityRole.DIRECTOR.check(role) && !SolidityRole.MEMBER.check(role)) {
            val account = Account(
                ...
                field0 = "0",
                membershipClaim = "",
                role = SolidityRole.MEMBER.value and SolidityRole.DIRECTOR.value
            )
            val claim = account.toClaim()
            account.membershipClaim = Hash.sha3String(claim)
